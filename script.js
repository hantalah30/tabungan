document.addEventListener("DOMContentLoaded", () => {
  ...
  let appState = {
    settings: {
      ...
      weddingDate: new Date(new Date().setFullYear(new Date().getFullYear() + 1)).toISOString().slice(0, 16),
  ...
  function initFirestoreListeners() {
    ...
    collections.transactions.orderBy("createdAt", "desc").onSnapshot((snapshot) => {
    ...
    collections.budget.orderBy("createdAt", "asc").onSnapshot((snapshot) => {
      appState.budgetItems = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
      updateAllUI();
    });

    // Listener for Vendors
    collections.vendors.orderBy("createdAt", "asc").onSnapshot((snapshot) => {
      appState.vendors = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
      renderVendorsPage();
    });
  }
  ...
  function renderHomePage() {
    ...
    const progress = Math.min((balance / appState.settings.targetSavings) * 100, 100);
    ...
    document.getElementById("home-budget-summary").innerHTML = `
        ...
        <div class="overview-grid">
            <div><span>Dialokasikan</span><strong>${formatToRupiah(totalAllocated)}</strong></div>
            <div><span>Dibelanjakan</span><strong class="expense">${formatToRupiah(totalSpent)}</strong></div>
            <div><span>Sisa</span><strong>${formatToRupiah(totalAllocated - totalSpent)}</strong></div>
        </div>`;

    // Render Next Milestone
    const nextMilestone = appState.milestones.find((m) => balance < m.amount);
    document.getElementById("next-milestone-card").innerHTML = nextMilestone ? `🏆 Target Berikutnya: ${nextMilestone.name}` : "🎉 Semua target tercapai!";
    ...
    appState.transactions.forEach((t) => {
      ...
      item.innerHTML = `
        ...
        <div class="details">
          ...
          ${t.type === 'expense' && category ? `<span class="category-badge">${category.categoryName}</span>` : ''}
        ...
        <div class="transaction-actions">
          ...
          <button class="action-btn delete-btn" data-id="${t.id}" title="Hapus">🗑️</button>
        </div>`;
  ...
  function renderBudgetPage() {
      ...
      document.getElementById('budget-overview-card').innerHTML = `
          <div class="overview-grid">
              <div><span>Total Anggaran</span><strong>${formatToRupiah(totalAllocated)}</strong></div>
              <div><span>Total Terpakai</span><strong class="expense">${formatToRupiah(totalSpent)}</strong></div>
              <div><span>Sisa Anggaran</span><strong>${formatToRupiah(totalAllocated - totalSpent)}</strong></div>
          </div>`;
      ...
      appState.budgetItems.forEach(item => {
          ...
          const progress = item.allocatedAmount > 0 ? (spentOnCategory / item.allocatedAmount) * 100 : 0;
          ...
          budgetLi.innerHTML = `
              ...
              <div class="progress-bar-container">
                  <div class="progress-bar" style="width: ${progress}%; background-color: ${progress > 100 ? 'var(--expense-color)' : 'var(--accent-color)'};"></div>
              </div>
              <div class="budget-item-details">
                  <span>Terpakai: <span class="expense">${formatToRupiah(spentOnCategory)}</span></span>
                  <span>Sisa: ${formatToRupiah(remaining)}</span>
              </div>
              <div class="budget-item-actions">
                  <button class="action-btn edit-budget-btn" data-id="${item.id}" title="Edit">✏️</button>
                  <button class="action-btn delete-budget-btn" data-id="${item.id}" title="Hapus">🗑️</button>
              </div>
          `;
      ...
      appState.vendors.forEach(vendor => {
          ...
          vendorLi.innerHTML = `
              ...
              <div class="vendor-contact">
                  ${vendor.phone ? `<span>📞 <a href="tel:${vendor.phone}">${vendor.phone}</a></span>` : ''}
                  ${vendor.email ? `<span>📧 <a href="mailto:${vendor.email}">${vendor.email}</a></span>` : ''}
              </div>
              ${vendor.notes ? `<div class="vendor-notes">${vendor.notes.replace(/\n/g, '<br>')}</div>` : ''}
              <div class="vendor-item-actions">
                  <button class="action-btn edit-vendor-btn" data-id="${vendor.id}" title="Edit">✏️</button>
                  <button class="action-btn delete-vendor-btn" data-id="${vendor.id}" title="Hapus">🗑️</button>
              </div>
          `;
  ...
  function renderSettingsPage() {
      document.getElementById('setting-name1').value = appState.settings.names.person1;
      document.getElementById('setting-name2').value = appState.settings.names.person2;
      document.getElementById('setting-target').value = appState.settings.targetSavings;
      document.getElementById('setting-date').value = appState.settings.weddingDate;
  }
  ...
  function renderAnalysisPage() {
      ...
      const chartOptions = {
          responsive: true,
          plugins: { legend: { position: 'top', labels: { color: chartTextColor } }, title: { display: true, color: chartTextColor, font: { size: 16 } } },
      };
      ...
      charts.contribution = new Chart(contributionCtx, {
          ...
          data: {
              ...
              datasets: [{
                  label: 'Total Kontribusi', data: Object.values(contributionData),
                  backgroundColor: ['#f06292', '#64b5f6', '#81c784', '#ffd54f'],
                  borderColor: 'rgba(255,255,255,0.2)', borderWidth: 2,
              }]
          },
          options: { ...chartOptions, plugins: { ...chartOptions.plugins, title: { ...chartOptions.plugins.title, text: 'Total Uang Ditabung per Orang' } } }
      });
      ...
      charts.spending = new Chart(spendingCtx, {
          ...
          data: {
              ...
              datasets: [{
                  label: 'Total Pengeluaran', data: spendingData.map(d => d.spent),
                  backgroundColor: '#ef5350',
              }]
          },
          options: { ...chartOptions, plugins: { ...chartOptions.plugins, title: { ...chartOptions.plugins.title, text: 'Pengeluaran per Kategori' } } }
      });

      // Chart 3: Savings Growth Over Time
      const savingsHistory = [];
      let currentBalance = 0;
      [...appState.transactions].reverse().forEach(t => { // oldest first
          currentBalance += (t.type === 'income' ? t.amount : -t.amount);
          savingsHistory.push({ x: t.createdAt.toDate(), y: currentBalance });
      });
      const savingsCtx = document.getElementById("savings-over-time-chart").getContext("2d");
      if (charts.savingsOverTime) charts.savingsOverTime.destroy();
      charts.savingsOverTime = new Chart(savingsCtx, {
          type: 'line',
          data: {
              datasets: [{
                  label: 'Pertumbuhan Tabungan', data: savingsHistory,
                  borderColor: 'var(--accent-color)', tension: 0.1,
              }]
          },
          options: {
              ...chartOptions,
              scales: { x: { type: 'time', time: { unit: 'month' }, ticks: { color: chartTextColor } }, y: { ticks: { color: chartTextColor } } },
              plugins: { ...chartOptions.plugins, title: { ...chartOptions.plugins.title, text: 'Pertumbuhan Tabungan' } }
          }
      });
  }
  ...
  const calculateBalance = (transactions) => transactions.reduce((acc, t) => (t.type === "income" ? acc + t.amount : acc - t.amount), 0);
  const calculateBudgetSummary = () => {
      const totalAllocated = appState.budgetItems.reduce((acc, item) => acc + item.allocatedAmount, 0);
  ...
  function openModal(modalId, data = null) {
      ...
      if (modalId === 'transaction-modal') {
          ...
          form.reset();
          document.getElementById('transaction-modal-title').textContent = data ? "Edit Transaksi" : "Tambah Transaksi";
          ...
          document.getElementById('budget-modal-title').textContent = data ? 'Edit Kategori' : 'Tambah Kategori';
          if(data) {
              document.getElementById('budget-id').value = data.id;
              document.getElementById('budget-category-name').value = data.categoryName;
          ...
          document.getElementById('vendor-modal-title').textContent = data ? 'Edit Vendor' : 'Tambah Vendor';
           if(data) {
  ...
  function toggleTransactionFormFields() {
      ...
      document.getElementById('note-wrapper').style.display = type === 'income' ? 'block' : 'none';
  }
  ...
  function setupEventListeners() {
    ...
    // Sub-page navigation (back buttons and links)
    ...
    document.getElementById("add-vendor-btn").addEventListener("click", () => openModal('vendor-modal'));
    ...
    document.getElementById("transaction-list").addEventListener("click", handleListActions);
    document.getElementById("budget-list").addEventListener("click", handleListActions);
    document.getElementById("vendor-list").addEventListener("click", handleListActions);
    ...
    document.getElementById("type").addEventListener("change", toggleTransactionFormFields);
  }
  ...
  function handleTransactionSubmit(e) {
    ...
    const transactionData = {
      ...
      category: type === "expense" ? document.getElementById("transaction-category").value : null,
    };

    const promise = id
      ? collections.transactions.doc(id).update(transactionData)
      : collections.transactions.add({ ...transactionData, createdAt: firebase.firestore.FieldValue.serverTimestamp() });

    promise.then(() => {
        showToast(id ? "Berhasil diperbarui!" : "Berhasil disimpan!", "success");
  ...
  function handleBudgetSubmit(e) {
      ...
      const budgetData = {
          ...
          allocatedAmount: parseFloat(document.getElementById('budget-amount').value),
      };
      const promise = id
        ? collections.budget.doc(id).update(budgetData)
        : collections.budget.add({ ...budgetData, createdAt: firebase.firestore.FieldValue.serverTimestamp() });

      promise.then(() => {
        showToast(id ? "Kategori diperbarui!" : "Kategori disimpan!", "success");
  ...
  function handleVendorSubmit(e) {
      ...
      const vendorData = {
       ...
       const promise = id
        ...
        : collections.vendors.add({ ...vendorData, createdAt: firebase.firestore.FieldValue.serverTimestamp() });
  ...
  function handleSettingsSubmit(e) {
      ...
      const settingsData = {
          ...
          targetSavings: parseFloat(document.getElementById('setting-target').value),
          weddingDate: document.getElementById('setting-date').value,
      };
      collections.settings.set(settingsData, { merge: true })
  ...
  function handleListActions(e) {
      ...
      if(list === 'transaction-list') {
          ...
          if (btn.classList.contains("delete-budget-btn")) {
              if (confirm("Yakin ingin menghapus kategori ini? Semua transaksi terkait tidak akan terhapus, namun kategorinya akan dihilangkan.")) {
    ...
    if (window.matchMedia("(pointer: fine)").matches) {
      cards.forEach((card) => {
        card.addEventListener("mousemove", (e) => {
          ...
          card.style.transform = `perspective(1000px) rotateX(${-y/30}deg) rotateY(${x/30}deg) scale3d(1.05, 1.05, 1.05)`;
        });
  ...
  const formatDate = (timestamp) => timestamp ? new Date(timestamp.seconds * 1000).toLocaleDateString("id-ID", { day: "numeric", month: "short", year: "numeric" }) : "";
